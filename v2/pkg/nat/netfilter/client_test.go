...
func TestNatClient_SyncNat(t *testing.T) {
	...
	v4SubnetsSecond := []*net.IPNet{
		...
		{IP: net.ParseIP("9.9.9.9"), Mask: net.CIDRMask(24, 32)},
		{IP: net.ParseIP("0.0.0.0"), Mask: net.CIDRMask(0, 32)},
	}
...
func checkOriginatingOnlyNFTablesRules(link netlink.Link, family int) error {
	...
	table, err := conn.ListTableOfFamily(mangleTable, nf)
	if err != nil {
		return fmt.Errorf("failed to get mangle table: %w", err)
	}

	input, err := conn.ListChain(table, strings.ToLower(inputChain))
	if err != nil {
		return fmt.Errorf("failed to get input chain in table %q: %w", table.Name, err)
	}

	output, err := conn.ListChain(table, strings.ToLower(outputChain))
	if err != nil {
		return fmt.Errorf("failed to get output chain in table %q: %w", table.Name, err)
	}

	deviceSet, err := conn.GetSetByName(table, link.Attrs().Name)
	if err != nil {
