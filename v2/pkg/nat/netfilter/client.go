...
func (n *NatClient) IsInitialized() (bool, error) {
	...
	if n.ipv6 != nil {
		if ok, err := isRuleInitialized(netlink.FAMILY_V6, n.v4InCluster); err != nil {
			return false, fmt.Errorf("failed to check IPv4 rule initialization: %w", err)
		} else if !ok {
			return false, err
		}
...
func (n *NatClient) initRules(family int) error {
	...
	narrowRule := newRule(netlink.FAMILY_V4, ncNarrowTableID, ncNarrowPrio)
	...
	wideRule := newRule(netlink.FAMILY_V4, ncWideTableID, ncWidePrio)
...
func (n *NatClient) addRoute(link netlink.Link, ipn *net.IPNet) error {
	...
	for _, cipn := range inCluster {
		...
		if err := netlink.RouteAdd(&netlink.Route{
			...
			return fmt.Errorf("netlink: failed to add route(table %d) to %s: %w", ncNarrowTableID, cipn.String(), err)
		}
		return nil
	}
