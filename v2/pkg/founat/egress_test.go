...
func TestEgress(t *testing.T) {
	...
	t.Run("NFTables-Dual", testEgressNFTablesDual)
	t.Run("NFTables-IPv4", testEgressNFTablesV4)
	t.Run("NFTables-IPv6", testEgressNFTablesV6)
}
...
func testEgressNFTablesDual(t *testing.T) {
	t.Parallel()

	eNS, err := ns.GetNS("/run/netns/test-egress-nft-dual")
	if err != nil {
		t.Fatal(err)
	}
	defer eNS.Close()
...
func testEgressNFTablesV4(t *testing.T) {
	t.Parallel()

	eNS, err := ns.GetNS("/run/netns/test-egress-nft-v4")
	if err != nil {
		t.Fatal(err)
	}
	defer eNS.Close()
...
func testEgressNFTablesV6(t *testing.T) {
	t.Parallel()

	eNS, err := ns.GetNS("/run/netns/test-egress-nft-v6")
	if err != nil {
		t.Fatal(err)
	}
	defer eNS.Close()
