...
var _ = Describe("Coild server", func() {
	...
	BeforeEach(func() {
		...
		af := func(interfaces map[string]bool, conf *nodenet.PodNetConf, logger *uberzap.Logger, pod *corev1.Pod) error {
			return nil
		}

		serv := NewCoildServer(l, mgr, nodeIPAM, podNet, natsetup, cfg, logger, af)
		err = mgr.Add(serv)
		Expect(err).ToNot(HaveOccurred())
